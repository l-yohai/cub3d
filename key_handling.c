...
/*   Updated: 2020/07/06 09:25:01 by yohlee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_player(t_info *info, t_player *player, double move_speed)
{
	if (info->map[(int)(player->x +\
					player->dir_x * move_speed)][(int)player->y] != '1')
		player->x += player->dir_x * move_speed;
	if (info->map[(int)player->x][(int)(player->y +\
									player->dir_y * move_speed)] != '1')
		player->y += player->dir_y * move_speed;
}

void
	move_player_left_right(t_info *info, t_player *player, double move_speed)
{
